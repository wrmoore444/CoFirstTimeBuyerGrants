...
export default async function WhoQualifiesPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params
  if (!isValidLang(lang)) notFound()
  const dict = getDictionary(lang)
  const h = dict.whoQualifies
  ...
  return (
    <>
      ...
      <ContentSection background="white">
        <BenefitsGrid items={h.criteria} columns={2} variant="card" />
        <p className="mt-8 text-sm text-muted-foreground text-center italic">{h.disclaimer}</p>
      </ContentSection>
